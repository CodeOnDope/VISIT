...
def setup_detector_logger(detector_name, level=logging.INFO):
    """Set up a logger for a detector.
    ...
    Returns:
        logging.Logger: Configured detector logger
    """
    # Get project root directory (assumed to be 2 levels up from this file)
    current_dir = os.path.dirname(os.path.abspath(__file__))
    project_root = os.path.abspath(os.path.join(current_dir, "..", ".."))
    
    # Create detector logs directory if it doesn't exist
    logs_dir = os.path.join(project_root, "logs", "detection")
    os.makedirs(logs_dir, exist_ok=True)
    
    # Create log file name with timestamp
    timestamp = datetime.datetime.now().strftime("%Y%m%d")
    log_file = os.path.join(logs_dir, f"{detector_name}_{timestamp}.log")
    
    # Set up the logger
    logger_name = f"Detector.{detector_name}"
    return setup_logger(logger_name, level=level, log_file=log_file)


def setup_performance_logger(component_name, level=logging.INFO):
    """Set up a logger for performance tracking.
    ...
    logs_dir = os.path.join(project_root, "logs", "performance")
    os.makedirs(logs_dir, exist_ok=True)
    
    # Create log file name with timestamp
    timestamp = datetime.datetime.now().strftime("%Y%m%d")
    log_file = os.path.join(logs_dir, f"perf_{component_name}_{timestamp}.log")
    
    # Set up the logger
    logger_name = f"Performance.{component_name}"
    return setup_logger(logger_name, level=level, log_file=log_file)
